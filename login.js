...
// Envia o e-mail de verificação após o registro
async function sendVerificationEmail(user) {
  try {
    await sendEmailVerification(user);
    showSuccessMessage('Verificamos seu cadastro! Verifique seu e-mail antes de fazer login.');
  } catch (error) {
    displayErrorMessage(error.message);
  }
}

// Atualiza a função registerUser para incluir a verificação de e-mail
async function registerUser(email, password, username) {
  try {
    const userCredential = await createUserWithEmailAndPassword(auth, email, password);
    const user = userCredential.user;
    ...
    console.log('Usuário cadastrado com sucesso:', user);
    await sendVerificationEmail(user); // Envia e-mail de verificação
    auth.signOut(); // Faz logout para impedir login antes da verificação

    showSuccessMessage('Cadastro realizado! Verifique seu e-mail para ativar a conta.');
  } catch (error) {
    displayErrorMessage(error.message);
  }
}

// Atualiza a função loginUser para impedir login de usuários não verificados
async function loginUser(email, password) {
  try {
    const userCredential = await signInWithEmailAndPassword(auth, email, password);
    const user = userCredential.user;
    ...
    if (!user.emailVerified) {
      await auth.signOut(); // Impede o acesso
      displayErrorMessage('Você precisa verificar seu e-mail antes de continuar.');
      return;
    }
